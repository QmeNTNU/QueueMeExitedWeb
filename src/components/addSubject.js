...
import { favoriteStudentSubjectListFetch, getAddedSubject, deleteSubject, addSubject1 } from '../actions';
...
class addSubject extends Component {
  ...
  componentDidMount() {
    ...
    if (user) {
      ...
      const list = this.props.favoriteStudentSubjectList;
      const type ='student';
...
//console.log('CHOOSESUBJECT RENDERED BUT WITHOUT LOGIN');
  ...
  findNewArray() {
    ...
    const favoriteStudentSubjectList = this.props.favoriteStudentSubjectList;
    for (let i = 0; i < addSubjectArray.length; i++) {
      for (let j = 0; j < favoriteStudentSubjectList.length; i++) {
      if(favoriteStudentSubjectList[j].emnekode === addSubjectArray[i].emnekode){
  ...
  renderList() {
    ...
    if (this.state.addOrDelete === 'delete') {
      ...
      list=this.props.favoriteStudentSubjectList
...
const mapStateToProps = state => {
  ...
  return { favoriteStudentSubjectList, addSubjectArray, addSubject };
};

export default connect(mapStateToProps,
  { favoriteStudentSubjectListFetch, getAddedSubject, deleteSubject, addSubject1 })(addSubject);
